...
$DESCRIPTION = "Nagios Plugin to check for Ambari hosts alerts via Ambari REST API

Checks whether there are CRITICAL / WARNING / UNKNOWN alerts for hosts in Ambari and returns the corresponding status output and exit code.
...
$msg = "Ambari host alerts: ";
my $msg2 = "";
$json = curl_ambari "$url_prefix/clusters/$cluster?fields=alerts_summary_hosts";
my %alerts_summary = get_field_hash("alerts_summary_hosts");
#my @alert_categories = qw/CRITICAL WARNING UNKNOWN OK MAINTENANCE/;
# There is no MAINTENANCE category for hosts
my @alert_categories = qw/CRITICAL WARNING UNKNOWN OK/;
