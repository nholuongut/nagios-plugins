...
Nagios Plugin to check Blue Talon masking functions via the Policy Management server REST API

Currently only determines the number of masking functions, also outputs perfdata.

Warning: the API doesn't expose a count so this program must fetch and count all masking functions
which is an O(n) operation and as such the performance get worse the more masking functions you have.

Optional thresholds may be applied to the number of masking functions
...
class CheckBlueTalonMaskingFunctions(NagiosPlugin):

    def __init__(self):
        # Python 2.x
        super(CheckBlueTalonMaskingFunctions, self).__init__()
        # Python 3.x
        # super().__init__()
    ...
    def run(self):
        ...
        url = '{protocol}://{host}:{port}/PolicyManagement/{api_version}/masking_functions'\
        ...
        try:
            ...
            masking_functions_list = json_dict['masking_functions']
            if not isList(masking_functions_list):
                raise ValueError("non-list returned for 'masking_functions' key by Blue Talon API (got type '{0}')"\
                                 .format(type(masking_functions_list)))
            num_masking_functions = len(masking_functions_list)
            self.msg += '{num_masking_functions} masking functions'\
                        .format(num_masking_functions=num_masking_functions)
            self.check_thresholds(num_masking_functions)
            self.msg += ' | num_masking_functions={num_masking_functions}{perf}'\
                        .format(num_masking_functions=num_masking_functions,
