...
Nagios Plugin to check Blue Talon policies via the Policy Management server REST API

Currently only determines the number of policies, also outputs perfdata.

Warning: the API doesn't expose a count so this program must fetch and count all policies
which is an O(n) operation and as such the performance get worse the more policies you have.

Optional thresholds may be applied to the number of policies
...
class CheckBlueTalonPolicies(NagiosPlugin):

    def __init__(self):
        # Python 2.x
        super(CheckBlueTalonPolicies, self).__init__()
        # Python 3.x
        # super().__init__()
    ...
    def run(self):
        ...
        url = '{protocol}://{host}:{port}/PolicyManagement/{api_version}/policies'\
        ...
        try:
            ...
            if not isList(policies_list):
                raise ValueError("non-list returned for 'policies' key by Blue Talon API (got type '{0}')"\
                                 .format(type(policies_list)))
            num_policies = len(policies_list)
            self.msg += '{0} policies'.format(num_policies)
            self.check_thresholds(num_policies)
            self.msg += ' | num_policies={num_policies}{perf}'.format(num_policies=num_policies,
