...
class CheckTravisCILastBuild(NagiosPlugin):
    ...
    def add_options(self):
        self.add_opt('-r', '--repo', default=os.getenv('TRAVIS_REPO'),
                     help="Travis repo ($TRAVIS_REPO, 'user/repo' eg. 'nholuongut/nagios-plugins'" + \
    ...
    def get_latest_build(self, content):
        ...
        if not builds:
            qquit('UNKNOWN', "no Travis CI builds returned by the Travis API."
                  ...
                  + " Also remember the repo is case sensitive, for example 'nholuongut/nagios-plugins' returns this"
                  + " blank build set whereas 'nholuongut/nagios-plugins' succeeds"
